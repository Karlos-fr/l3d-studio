...
Créer une version modulaire et mesurable de `SparkPixelsMega` pour le Particle
Photon Gen 2. La source upstream utilisée pour l'import initial est désormais
identifiée par son empreinte dans `docs/BASELINE.md`.
...
- Bibliothèque LED canonique : `firmware/src/platform/neopixel.cpp` et
  `firmware/src/platform/neopixel.h`.
- Source upstream initiale : archive Spark Pixels identifiée par le SHA-256
  conservé dans `docs/BASELINE.md`, puis retirée après le refactor.
...
- [x] Copier la source upstream dans `firmware/src/main.cpp` sans modification fonctionnelle.
- [x] Copier la bibliothèque NeoPixel corrigée dans `firmware/src/platform/`.
...
## Phase 9 — Métadonnées compactes et protocole v2 (abandonnée)

> Décision du 17 août 2026 : le prototype a confirmé un gain limité à environ
> 2 Ko de RAM statique et 4 Ko de Flash dans une variante supprimant totalement
> les métadonnées historiques. Ce gain ne justifie pas le registre de modes
> dupliqué dans L3D Studio, la perte de découverte dynamique ni le couplage des
> versions du firmware et de l'application web. La phase reste ci-dessous comme
> trace de l'étude, mais ses tâches ne sont plus prévues.
...
- [ ] Conserver les empreintes de la source upstream dans la baseline.
